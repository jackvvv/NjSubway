...
import com.itpoints.njmetro.ui.ad.AdDetailUI;
...
public class AppNotifier {
	...
	public synchronized void onNewMsg(MsgBean msgBean) {
		...
		// 判断app是否在后台
		if (!EasyUtils.isAppRunningForeground(appContext) && isScreenOn()) {
	...
	protected void sendNotification(MsgBean msgBean, boolean isForeground, boolean numIncrease) {
		try {
			...
			if ("1".equals(msgBean.getType())) {
				msgIntent = new Intent(appContext, AdDetailUI.class);
				msgIntent.putExtra("url", msgBean.getUrl());
			} else {
			...
			if ("1".equals(type)) {
				mBuilder.setContentText(msgBean.getUrl());
			} else if ("2".equals(type)) {
				mBuilder.setContentText(msgBean.getContent());
			}
	...
	public interface AppNotificationInfoProvider {
		 ...
		 * @return null为使用默认
		 */
		String getDisplayedText(MsgBean msgBean);
